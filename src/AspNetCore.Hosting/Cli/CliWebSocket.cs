...
{
    internal class CliWebSocket : WebSocket
    {
        ...
        public static Tuple<CliWebSocket, CliWebSocket> CreatePair(string subProtocol)
        {
            var buffers = new[] { new ReceiverSenderBuffer(), new ReceiverSenderBuffer() };
            return Tuple.Create(
                new CliWebSocket(subProtocol, buffers[0], buffers[1]),
                new CliWebSocket(subProtocol, buffers[1], buffers[0]));
        }
        ...
        private void ThrowIfDisposed()
        {
            if (_state >= WebSocketState.Closed) // or Aborted
            {
                throw new ObjectDisposedException(typeof(CliWebSocket).FullName);
            }
        }
        ...
        private CliWebSocket(string subProtocol, ReceiverSenderBuffer readBuffer, ReceiverSenderBuffer writeBuffer)
        {
        ...
        private class ReceiverSenderBuffer
        {
            ...
            public virtual Task SendAsync(Message message, CancellationToken cancellationToken)
            {
                lock (_messageQueue)
                {
                    ...
                    if (_receiverClosed)
                    {
                        throw new IOException("The remote end closed the connection.", new ObjectDisposedException(typeof(CliWebSocket).FullName));
                    }
